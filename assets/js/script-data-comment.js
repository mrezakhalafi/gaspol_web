function getDisplayName(fPin, index) {
    ...
    if (name == fPin) {
        ...
        xmlHttp.open("get", "/gaspol_web/logics/fetch_person?f_pin=" + fPin);
        xmlHttp.send();
    } else {
...
function getThumbId(fPin, index) {
    ...
    if (thumb == '') {
        ...
        xmlHttp.onreadystatechange = function () {
            if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
                ...
                if (personData.length > 0) {
                    var person = personData[0];
                    if (person.IMAGE) {
                        thumb = 'http://108.136.138.242/filepalio/image/' + person.IMAGE;
                    }
                    document.getElementById('user-thumb-' + index).src = thumb;
                }
            }
        ...
        thumb = 'http://108.136.138.242/filepalio/image/' + thumb;
        document.getElementById('user-thumb-' + index).src = thumb;
    }
}
