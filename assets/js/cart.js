    ...
    if (saved.length > 0) {
        ...
        saved.slice().reverse().forEach(item => {
            ...
            let html_shop =
                ...
                `<div class="col-2"><img class="verified" src="../assets/img/icons/Verified-(Black).png"></div><div class="col-10 px-1">${merchant_name}</div>` +
                '</div>' +
                '</div>' +
                ...
                '<img class="view-store" src="../assets/img/tab5/store-black.webp"> ' + viewstore +
                '</a>' +
                '</div>' +
                '</div>' +
            ...
            item.items.forEach(item => {
                ...
                if (!thumb_src.includes("/gaspol_web/images/")) {
                    thumb_src = "/gaspol_web/images/" + thumb_src;
                }
                ...
                let html_item =
                    ...
                    `<div class="row">` +
                    `<div class="col-5">` +
                    '<div class="input-group counter mt-2" style="width: 85px;">' +
                    ...
                    `<img onclick="deleteItem('${merchant_name}', '${item.itemName}', 'saved');" class="delete-icon" src="../assets/img/cart/Delete.webp">` +
                    '</div>' +
                    '</div>' +
                    '</div>';
        ...
        unsaved.slice().reverse().forEach(item => {
            ...
            item.items.forEach(item => {
                ...
                let html_item =
                    ...
                    '<div class="col-1 col-delete">' +
                    '<div class="delete-btn">' +
                    `<img onclick="deleteItem('${merchant_name}', '${item.itemName}', 'cart');" class="delete-icon" src="../assets/img/cart/Delete.webp">` +
                    '</div>' +
                    '</div>' +
                    '</div>';
...
function displaySuccess(creditCardCharge) {
    console.log('display', creditCardCharge);
            ...
            try {
                if (data.status == "CAPTURED") {
                    ...
                    postForm("../logics/paliobutton/php/store_payment", {
                        ...
                        status: "success",
                        items: base64items,
                        cart: utoa(JSON.stringify(purchased_cart))
                    });
                } else {
                    alert("Credit card transaction failed");
                    let fieldset = document.getElementById('fieldset-card');
...
async function palioPay() {
    ...
    // let stockValid = await checkStock();
...
function deliveryAddress(address = '') {
    ...
    return new Promise(function (resolve, reject) {
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/gaspol_web/logics/get_delivery_address");
