...
function getWishlist(id_user) {
    ...
    xhr.open("POST", "/gaspol_web/logics/get_wishlist");
    ...
    xhr.onload = function () {
        if (xhr.status != 200) { // analyze HTTP status of the response
            ...
            JSON.parse(responseObj).forEach(element => {
                ...
                +ext+` src="/gaspol_web/images/${element.THUMB_ID.split("|")[0].replace("http://202.158.33.26", "")}`+is_vid+`" class="wishlist-images">`+ext2+
...
function addWishlist(product_code, element) {
    ...
    let operation;
    if (star == `${document.location.origin}/gaspol_web/assets/img/icons/Wishlist-fill.png`) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "/gaspol_web/logics/add_to_wishlist");
...
function getWishlistQuery(id_user, query) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "/gaspol_web/logics/get_wishlist_query");
