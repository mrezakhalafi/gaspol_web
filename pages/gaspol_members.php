<?php
    ...
    $query = $dbconn->prepare("SELECT CLUB_MEMBERSHIP.*, USER_LIST.*, TKT.*, CLUB_MEMBERSHIP.F_PIN AS CLUB_F_PIN FROM CLUB_MEMBERSHIP LEFT JOIN USER_LIST ON USER_LIST.F_PIN = CLUB_MEMBERSHIP.F_PIN LEFT JOIN TKT ON TKT.ID = CLUB_MEMBERSHIP.CLUB_CHOICE WHERE CLUB_MEMBERSHIP.STATUS = 1 AND CLUB_CHOICE = '".$f_pin."'");
    $query->execute();
    $memberData = $query->get_result();
    $query->close();
...
<html>
  <head>
    ...
    <div class="p-3 shadow-sm" style="border-bottom: 1px solid #e4e4e4">
        <div class="row">
            ...
            <div class="col-10 pt-1">
                <b style="font-size: 14px"><?= mysqli_num_rows($memberData) ?> Member</b>
            </div>
        </div>
    </div>
    ...
    <div class="px-3 pt-2">
        ...
        <?php if (mysqli_num_rows($memberData) > 0): ?>
            <?php foreach($memberData as $fd): ?>

                <div class="row pt-3">
                    <div class="col-2" onclick="openProfile('<?= $fd['CLUB_F_PIN'] ?>')">
                    ...
                    <div class="col-8" onclick="openProfile('<?= $fd['CLUB_F_PIN'] ?>')">
                        <div style="font-size: 13px">
                            <b><?= $fd['FIRST_NAME']." ".$fd['LAST_NAME'] ?></b>
                        </div>
                        <div style="font-size: 12px"><span style="color: darkorange">
                            Member </span>of <?= $fd['CLUB_NAME'] ?>
                        </div>
                    </div>
                    ...
                    <?php 
                        ...
                        ?>
                            ...
                            <div class="col-2">
                                <img id="icon-follow-<?= $fd['CLUB_F_PIN'] ?>" src="../assets/img/social/follow.svg" style="height: 30px; width: 30px" onclick="follow('<?= $fd['CLUB_F_PIN'] ?>')">
                                <img id="icon-unfollow-<?= $fd['CLUB_F_PIN'] ?>" class="d-none" src="../assets/img/social/followed.svg" style="height: 30px; width: 30px" onclick="unfollow('<?= $fd['CLUB_F_PIN'] ?>')">
                            </div>

                        <?php else: ?>

                            <div class="col-2">
                                <img id="icon-follow-<?= $fd['CLUB_F_PIN'] ?>" class="d-none" src="../assets/img/social/follow.svg" style="height: 30px; width: 30px" onclick="follow('<?= $fd['CLUB_F_PIN'] ?>')">
                                <img id="icon-unfollow-<?= $fd['CLUB_F_PIN'] ?>" src="../assets/img/social/followed.svg" style="height: 30px; width: 30px" onclick="unfollow('<?= $fd['CLUB_F_PIN'] ?>')">
                            </div>
