...
function fetchProducts() {
    ...
    xmlHttp.open("get", "/gaspol_web/logics/fetch_posts_profile?f_pin=" + store_id);
    xmlHttp.send();
}
    ...
    return new Promise(function (resolve, reject) {
        ...
        xhr.onload = function () {
            if (this.status >= 200 && this.status < 300) {
                resolve({thumb_id:JSON.parse(xhr.response).THUMB_ID, name:JSON.parse(xhr.response).NAME, description:JSON.parse(xhr.response).DESCRIPTION});
            } else {
                reject({
                    status: this.status,
        ...
        if (thumbs.length == 1) {
            ...
            if (video_arr.includes(type)) {
                content = `
                    ...
                    <source src="../images/${thumbs[0]}" type="video/${type}">
                    </video>
                `;
