...
WHERE p.FPIN = '$f_pin' AND pr.CODE IS NOT NULL AND s.IS_QIOSK = 4");
$query->execute();
...
WHERE p.FPIN = '$f_pin' AND s.IS_QIOSK = 4");
$query->execute();
...
WHERE w.FPIN = '$f_pin' AND w.IS_POST = 0 AND pr.CODE IS NOT NULL AND s.IS_QIOSK = 4");
$query->execute();
$results = $query->get_result();
$query->close();
            ...
            <div class="row my-2 collection-row ps-2 slick-row">
                <div class="col-3 my-3 collections-left d-flex justify-content-center align-items-center">
                    ...
                    if ($col['THUMB_ID'] != null) {
                        ...
                        if (strpos($thumb, "/gaspol_web/images/") === false) {
                            $thumb = "/gaspol_web/images/" . $thumb;
                        }
